    ...
    function SuccPaymentData($data){
        ...
        foreach($GetAllSevaPrices as $SevaPrice){
            ...
            try{
                ...
                $seva=$SevaPrice['seva_price']['seva']['title'];
                ...
                $IsEvent = EventSeva::where('seva_id','=',$seva_id)->with('event:id,slug')->first();
                if(is_null($IsEvent)){
                    $sevaSlug = @$SevaPrice['seva_price']['seva']['slug'];
                    if(is_null($sevaSlug) || $sevaSlug==""){
                        $event_link = WEB_API_LINK().'seva/'.$seva_id;
                    }else{
                        $event_link = WEB_API_LINK().'seva/'.$sevaSlug;
                    }
                }else{
                    $EventSlug = @$IsEvent['event']->slug;
                    if(is_null($EventSlug) || $EventSlug==""){
                        $event_id   = $IsEvent->event_id;
                        $event_link = WEB_API_LINK().'sevas/'.$event_id;
                    }else{
                        $event_link = WEB_API_LINK().'sevas/'.$EventSlug;
                    }
                }
                $SbArr = array(
                    array(
            ...
            if($request->method()=="POST" || $request->method()=="PUT"){
                ...
                if($validator->fails()){
                    ...
                    if(!empty($request->all())){
                        ...
                        $booking_data = json_encode($ProdData);
                        try {
                            if($request->method()=="POST"){
                                if($userid>0){
                                    ...
                                    if(!empty($Cart)){
                                        ...
                                        $ProdData['booking_data'] = $booking_data;
                                        // $ProdData['order_id'] = $ProdData['reference_id'];
                                        $ProdData['created_at'] = date('Y-m-d H:i:s');
                                        $orderData = Order::create($ProdData);
                                        if($orderData->shipping_user_address_id){
                                            $UserAddress = UserAddress::withTrashed()->find($orderData->shipping_user_address_id);
                                        ...
                                        if($orderData->billing_user_address_id){
                                            $UserAddress = UserAddress::withTrashed()->find($orderData->billing_user_address_id);
                                            $UserAddress['state'] = State::find($UserAddress->state_id);
                                            $UserAddress['city'] = City::find($UserAddress->city_id);
                                            $UserAddress['country'] = Country::find($UserAddress->country_id);
                                            ...
                                            foreach($Cart as $ord){
                                                ...
                                                if($user_family_detail_id){
                                                    foreach($user_family_detail_id as $id){
                                                       $user_family_details =  UserFamilyDetail::with('rasi')->with('relation')->withTrashed()->find($id);
                                                        if(!is_null($user_family_details)){
                                                            $InstArr = array(
                                                                'user_family_detail_id'=>$id,
                                                                'family_details'=>$user_family_details,
