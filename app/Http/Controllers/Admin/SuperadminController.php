...
class SuperadminController extends Controller
{
	...
	public function order_old(){
		$status_id=0;
		$order_sevas = OrderSeva::with('seva_price')
		->with(["order" => function ($q) use ($status_id) {
		...
		->with('order_seva_family_details')
	...
	public function export_Order(Request $request){
		...
		$HeadData="Booking Id\tSeva\tPuja\tUserName\tMobile\tMail\tTrans.No\tName\tRelation\tDOB\tGothram\tRaasi\tNakshatram\tAddress 1\tAddress 2\tState\tCity\tPincode\tAddress Name\tBooking Date\tAmount\tStatus\n";
		if($order_sevas){
			$aaa=1;
			foreach($order_sevas as $sevas){
				if($sevas['Order']->payment_status!="Pending"){
					$BookingId=$sevas['Order']->invoice_id;

					$SevaTilte = $sevas['seva_price']['seva']->title;
					$SevaOp    = $sevas['seva_price']->title;
					...
					$decodestatus=1;
					$user_family 	= $sevas['order_seva_family_details'];
					$family_details = @$sevas['order_seva_family_details'][0]['user_family_detail'];
					if(is_null($family_details) || $family_details==""){
						$user_family = @$sevas['order_seva_family_details'][0]['user_family_detail'];
						$decodestatus=0;
					}else{
						$user_family = $family_details;
						$user_family = json_decode($user_family,true);
					}	
					$Name			=	@$user_family['full_name'];
					$Relation		=	@$user_family['relation']['name'];
					$DOB			=	@$user_family['dob'];
					$Gothram		=	@$user_family['gothram'];
					$Raasi			=	@$user_family['rasi']['name'];
					$Nakshatram		=	@$user_family['nakshatram'];
					...
					$payment_status	=	$sevas['Order']['payment_status'];
					$amount			=	$sevas['Order']['final_paid_amount'];
					
					$HeadData.=	"".$BookingId."\t".$SevaTilte."\t".$SevaOp."\t".$UserNam."\t".$Mobile."\t".$Mail."\t".$TransNo."\t".$Name."\t".$Relation."\t".$DOB."\t".$Gothram."\t".$Raasi."\t".$Nakshatram."\t".$Address1."\t".$Address2."\t".$State."\t".$City."\t".$Pincode."\t".$AddressName."\t".$BookingDate."\t".$amount."\t".$payment_status."\n";
				}
			}
		}
		echo $HeadData;exit;
